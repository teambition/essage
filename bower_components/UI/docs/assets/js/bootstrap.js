  ...
  * ========================== */
  ...
  $.fn.collapse = function (option) {
    return this.each(function () {
      ...
      if (!data) $this.data('collapse', (data = new Collapse(this, $.extend(options, {
        trigger: $('[href=#' + $this.attr('id') + ']')
      }))))
...
+function ($) { "use strict";
  ...
  var Dropdown = function (element, options) {
    ...
    $el.data('options', this.getOptions(options))
  }

  Dropdown.DEFAULTS = {
    direction: 'auto bottom'
  }
  ...
  Dropdown.prototype.toggle = function (e) {
    ...
    if (!isActive) {
      ...
      if ($parent.hasClass('open')) {
        setDirection($parent, $this.data('options') || Dropdown.DEFAULTS)
      }
    }
  ...
  function setDirection($parent, options) {
    ...
    if (autoPlace) {
      direction = direction.replace(autoToken, '') || 'bottom'
    }
    setCss()
    if (autoPlace) {
      ...
      if (offset.top < docScroll && direction == 'top') {
        direction = 'bottom'
      } else if (offset.top + dropdownHeight > docScroll + windowHeight && direction == 'bottom') {
        direction = 'top'
      }
      setCss()
    }
...
+function ($) { "use strict";
  ...
  Modal.prototype.show = function () {
    ...
    this.backdrop(function () {
      ...
      that.$element.show().trigger('showing.bs.modal')
...
+function ($) { "use strict";
  ...
  Tooltip.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      var calculatedOffset = this.getCalculatedOffset(placement, pos, actualWidth, actualHeight, docScroll, parentWidth, parentHeight, parentLeft)
  ...
  Tooltip.prototype.getCalculatedOffset = function (placement, pos, actualWidth, actualHeight, docScroll, parentWidth, parentHeight, parentLeft) {
    ...
    if (placement == 'right' || placement == 'left') {
      if (pos.top  + actualHeight - docScroll + viewportPadding > parentHeight) { // 如果popover的底部超出下边界
        pos.top = parentHeight + docScroll - actualHeight - viewportPadding
      } else if (pos.top - docScroll  - viewportPadding < 0) { // 如果popover顶部超出上边界
        pos.top = docScroll + viewportPadding
      }
    } else {
      if (pos.left + actualWidth + viewportPadding > parentWidth) { // 如果popover的右边超出右边界
        pos.left = parentWidth - actualWidth - viewportPadding
      } else if (pos.left - viewportPadding < parentLeft) { // 如果popover的左边超出左边界
        pos.left = parentLeft + viewportPadding
      }
    }
    return pos;
  }
...
+function ($) { "use strict";
  ...
  , trigger: 'click'
  , content: ''
  , template: '<div class="popover"><header class="popover-header">' +
                '<a class="popover-back-handler icon icon-chevron-left"></a>' +
                '<h3 class="popover-title">Popover Title</h3>' +
                '<a class="popover-close-handler icon icon-remove"></a>' +
              '</header><div class="popover-content"></div></div>'
  })
...
(function( $ ) {
  ...
  Datepicker.prototype = {
    ...
    fill: function() {
      ...
      this.picker.find('tfoot th.tomorrow')
        .text(dates[this.o.language].tomorrow)
        .toggle(this.o.tomorrowBtn!== false);
      this.picker.find('tfoot th.someday')
        .text(dates[this.o.language].someday)
        .toggle(this.o.somedayBtn!== false);
      this.updateNavArrows();
      this.fillMonths();
      var startDate = UTCDate(year, month, 1, 0,0,0,0);
      var prevMonthDays;
      if (month === 0) {
        prevMonthDays = DPGlobal.getDaysInMonth(year - 1, 11);
      } else {
        prevMonthDays = DPGlobal.getDaysInMonth(year, month - 1);
      }
      var currMonthDays = DPGlobal.getDaysInMonth(year, month);
      var endDate = UTCDate(year, month, currMonthDays, 0,0,0,0);
      if (startDate.getUTCDay() > 0) {
        startDate = UTCDate(year, month - 1, (prevMonthDays - startDate.getUTCDay() + 1), 0,0,0,0);
      }
      if (endDate.getUTCDay() < 6) {
        endDate = UTCDate(year, month + 1, (6 - endDate.getUTCDay()), 0,0,0,0)
      }
      var prevMonth = startDate;
      var html = [];
      var clsName;
      while(prevMonth.valueOf() <= endDate.valueOf()) {
  ...
  var DPGlobal = {
    ...
    headTemplate: '<thead>'+
              '<tr>'+
                '<th class="prev"><a class="icon icon-chevron-left"></span></a>'+
                '<th colspan="5" class="datepicker-switch"></th>'+
                '<th class="next"><a class="icon icon-chevron-right"></span></a>'+
              '</tr>'+
            '</thead>',
    contTemplate: '<tbody><tr><td colspan="7"></td></tr></tbody>',
    footTemplate: '<tfoot><tr><th colspan="3" class="today"></th><th colspan="2" class="tomorrow"></th><th colspan="2" class="someday"></th></tr></tfoot>'
  };
